...
- **A reindex may need a session restart to show up.** Observed against the real packaged app (not just the dev binary): after adding/reindexing a folder in the app, `reference-mcp` kept returning stale/empty results across several calls in the same session, then returned correct fresh results immediately after restarting the session. The `reference-mcp` process opens the on-disk LanceDB table once at startup — root cause not yet nailed down (possibly the `lancedb` `Table` handle not seeing newly committed writes without being reopened), but the practical fix today is the same one every MCP schema/tool change already needs: restart the session after reindexing if results look wrong.
