...
{
  "results": [
    { "path": "/abs/path/to/file.rs", "start_line": 154, "end_line": 251, "chunk_kind": "function", "score": 0.61, "content": "pub async fn hybrid_search(...) { ... }", "possibly_stale": false }
  ],
  "citations": [
...
Every `results` entry (not `citations` — those come from a separate synthesis struct that doesn't carry a stored `mtime`) also carries `possibly_stale`: `true` when the file's current on-disk mtime is newer than what the row was embedded from, computed fresh at query time (`mcp/src/main.rs`'s `is_possibly_stale`). This exists because live reindexing is best-effort, not instant — see "reindexing is best-effort" in the caveats below. `false` doesn't guarantee the content is current, only that nothing detectable is stale right now; if the path can't be stat'd at all (deleted, permission error), it's also reported `false` rather than treated as a stale signal.
...
- **Fixed in `85bfa45`: reads used to need a session restart to see a fresh reindex.** The `reference-mcp` process opens the on-disk LanceDB table once at startup and previously kept querying that same snapshot for the process's whole life, so writes committed by the app (adding/reindexing a folder) never showed up without restarting the session. `hybrid_search`/`find_similar`/`check_doc_drift` now call `table.checkout_latest()` (a cheap manifest read, not a rescan) before every read — see `core/src/store.rs`'s three call sites — so a reindex is visible on the *next* MCP call without restarting anything.
- **Reindexing itself is best-effort, not instant — treat it as eventually consistent.** Live reproduction (editing real watched folders and watching `core/src/watcher.rs`'s own log output) found that `notify`'s FSEvents delivery on macOS is genuinely non-deterministic under system load: the same edit, same code, sometimes reindexed near-instantly and sometimes took 30+ seconds, with no code-level bug behind the difference — root cause is OS-level event coalescing/latency, not something fixable in this codebase. Two things exist because of this, not to eliminate it: (1) `watch()` now registers its `notify` watch *before* the initial folder scan rather than after, so an edit made mid-scan on a large folder isn't lost forever waiting for the scan to finish; (2) a periodic full re-scan (`PERIODIC_RESCAN_INTERVAL`, every 5 minutes) re-walks each watched folder as a self-healing fallback, skipping any file whose on-disk mtime hasn't moved since it was last embedded (a stat, not a re-embed) so this stays cheap on large folders. Practical implication for an agent: don't assume a query immediately after an edit sees that edit — the `possibly_stale` field on every `search`/`find_similar`/`check_doc_drift` result (see that tool's Output section) is the mechanism to check this per-result instead of guessing, and building in a short wait before re-querying content you just changed is still the simplest mitigation.
