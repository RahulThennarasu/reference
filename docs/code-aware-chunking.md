...
status: **phases 1-4 implemented** (rust, python, typescript/javascript, go, java, c/c++, ruby, swift). this is a scoping doc, not an implementation plan to follow blindly, expect it to change once real code proves parts of it wrong. remaining languages (c#, php, kotlin — see gap #1, `docs/feature-gaps.md`) not started, picked up only as real need shows up.
...
5. **ruby, swift — done.** `core/src/chunk.rs` dispatches `"rb"` through `tree_sitter_ruby::LANGUAGE` and `"swift"` through `tree_sitter_swift::LANGUAGE`. Ruby's `method`/`singleton_method` nodes (instance methods and `def self.foo`/`def obj.foo` respectively) get the same container-swallowing treatment inside `class`/`module` bodies as every other language here — `singleton_class` (`class << self ... end`) is deliberately not matched, rare enough not to be worth a special case, and still gets indexed via its containing chunk rather than dropped. Swift's grammar unifies class/struct/enum/extension under one `class_declaration` node kind (distinguished only by a leaf keyword token, not a separate node type) and its `function_declaration` covers both free functions and methods nested in a class body, same shape as everything else; `protocol_declaration` is the interface-equivalent, and `init_declaration`/`deinit_declaration` are separate node kinds with no `name` field at all (just literal `init`/`deinit` tokens), so `extract_name` correctly returns `None` for those rather than guessing. Both verified against their grammars' own `tags.scm`/`node-types.json` (not docs.rs, which doesn't cover grammar node kinds), same verification standard as java/c/c++ above. Covered by unit tests in `chunk.rs`.
6. other languages (c#, php, kotlin — see gap #1, `docs/feature-gaps.md`), only if there's an actual need.
